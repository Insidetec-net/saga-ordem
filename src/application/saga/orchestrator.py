"""Saga Orchestrator — Coordinates distributed order processing."""
...
class OrderSagaOrchestrator:
    ...
    async def execute(self, order, idempotency_key: str | None = None):
        ...
        if idempotency_key:
            ...
            if not is_new:
                ...
                if cached and cached.get("status") == "completed":
                    logger.info(
                        f"Duplicate request {idempotency_key}, returning cached result"
                    )
                    order_id = UUID(cached["order_id"])
                    return await self.order_repo.get_by_id(order_id)
                raise SagaExecutionError(
                    f"Request {idempotency_key} already in progress"
                )
        ...
        try:
            ...
            logger.info(
                f"Saga {saga_id}: COMPLETED for order {order.order_id}"
            )
            return order

        except Exception as e:
            logger.error(
                f"Saga {saga_id}: FAILED at step "
                f"{completed_steps[-1].name if completed_steps else 'start'}: {e}"
            )
            await self.saga_log.log_step(
                order_id=order.order_id,
                saga_id=saga_id,
                step_name=(
                    completed_steps[-1].name if completed_steps else "start"
                ),
    ...
    async def _reserve_stock(
        ...
        for item in order.items:
            ...
            success = await self.stock_repo.update_with_version(
                stock, expected_version
            )
            if not success:
            ...
            reservations.append(
                {"product_id": str(item.product_id), "qty": item.quantity}
            )
    ...
    async def _confirm_order(
        ...
        await self.order_repo.update(order)
        await self.outbox.append(
            OrderConfirmed(order_id=order.order_id),
            order.order_id,
        )
    ...
    async def _compensate(
        ...
        logger.warning(
            f"Saga {saga_id}: starting compensation for order {order.order_id}"
        )
        ...
        try:
            ...
            logger.error(
                f"Failed to cancel order after compensation: {e}"
            )
    ...
    async def _release_stock(
        ...
        for item in order.items:
            ...
            if stock:
                ...
                await self.stock_repo.update_with_version(
                    stock, expected_version
                )
    ...
    async def _publish_events(self, order) -> None:
